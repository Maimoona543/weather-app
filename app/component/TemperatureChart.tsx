...
const TemperatureChart = ({ dates, tempMax, weatherCode }: Props) => {
  ...
  const pointStyles = weatherCode.map((code, i) =>
    i === 0 ? "circle" : createEmojiIcon(getWeatherIcon(code))
  );
  const pointRadii = weatherCode.map((_, i) => (i === 0 ? 8 : 18));

  // Customize today's circle color
  const pointBackgroundColors = weatherCode.map((_, i) =>
    i === 0 ? "#FFFFFF  " : "transparent"
  );
  const pointBorderColors = weatherCode.map((_, i) =>
    i === 0 ? "#FFFFFF " : "transparent"
  );
        ...
        labels: {
          day: {
            formatter: (_: string, ctx: { dataIndex: number }) => {
              const day = new Date(dates[ctx.dataIndex]).toLocaleDateString(
                "en-US",
                {
                  weekday: "short",
                }
